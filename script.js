document.addEventListener('DOMContentLoaded', () => {
    ...
    // COD Elements
    const dailyCodDetails = document.getElementById('dailyCodDetails');
    const dailyAddressInput = document.getElementById('dailyAddress');
    const dailyGetLocationBtn = document.getElementById('dailyGetLocationBtn');
    const dailyLocationStatus = document.getElementById('dailyLocationStatus');
    const dailyMapsLinkInput = document.getElementById('dailyMapsLink');
    const dailyServiceRadios = document.getElementsByName('serviceType');
    ...
    // Toggle COD Details
    dailyServiceRadios.forEach(radio => {
        radio.addEventListener('change', () => {
            if (radio.value === 'COD') {
                dailyCodDetails.style.display = 'block';
            } else {
                dailyCodDetails.style.display = 'none';
            }
        });
    });

    // Geolocation Logic (Daily)
    if (dailyGetLocationBtn) {
        dailyGetLocationBtn.addEventListener('click', () => {
            if (navigator.geolocation) {
                dailyLocationStatus.innerText = "Sedang mengambil lokasi...";
                dailyGetLocationBtn.disabled = true;
                ...
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        ...
                        dailyMapsLinkInput.value = mapsUrl;
                        dailyLocationStatus.innerHTML = `<i class="fas fa-check-circle"></i> Lokasi Terkunci!`;
                        dailyLocationStatus.style.color = "var(--color-accent)";
                        dailyGetLocationBtn.disabled = false;

                        // Open map in new tab for verification
                        window.open(mapsUrl, '_blank');
                    },
                    (error) => {
                        dailyLocationStatus.innerText = "Gagal mengambil lokasi. Pastikan GPS aktif.";
                        dailyLocationStatus.style.color = "red";
                        dailyGetLocationBtn.disabled = false;
    ...
    sendOrderBtn.addEventListener('click', () => {
        ...
        let addressStr = '';
        if (serviceType === 'COD') {
            const address = dailyAddressInput.value.trim();
            const mapsLink = dailyMapsLinkInput.value;

            if (!address) {
                alert('Mohon isi Alamat Lengkap untuk layanan COD!');
                dailyAddressInput.focus();
                return;
            }
            addressStr = `\nAlamat: ${address}`;
            if (mapsLink) {
    ...
    if (IS_RAMADAN) {
        ...
        // Bukber COD Elements
        const bukberCodDetails = document.getElementById('bukberCodDetails');
        const bukberAddressInput = document.getElementById('bukberAddress');
        const bukberGetLocationBtn = document.getElementById('bukberGetLocationBtn');
        const bukberLocationStatus = document.getElementById('bukberLocationStatus');
        const bukberMapsLinkInput = document.getElementById('bukberMapsLink');
        const bukberServiceRadios = document.getElementsByName('bukberServiceType');
        ...
        // Toggle Bukber COD Details
        if (bukberServiceRadios.length > 0) {
            bukberServiceRadios.forEach(radio => {
                radio.addEventListener('change', () => {
                    if (radio.value === 'COD') {
                        bukberCodDetails.style.display = 'block';
                    } else {
                        bukberCodDetails.style.display = 'none';
                    }
                });
            });
        }

        // Geolocation Logic (Bukber)
        if (bukberGetLocationBtn) {
            bukberGetLocationBtn.addEventListener('click', () => {
                if (navigator.geolocation) {
                    bukberLocationStatus.innerText = "Sedang mengambil lokasi...";
                    bukberGetLocationBtn.disabled = true;
                    ...
                    navigator.geolocation.getCurrentPosition(
                        (position) => {
                            ...
                            bukberMapsLinkInput.value = mapsUrl;
                            bukberLocationStatus.innerHTML = `<i class="fas fa-check-circle"></i> Lokasi Terkunci!`;
                            bukberLocationStatus.style.color = "var(--color-accent)";
                            bukberGetLocationBtn.disabled = false;

                            window.open(mapsUrl, '_blank');
                        },
                        (error) => {
                            bukberLocationStatus.innerText = "Gagal mengambil lokasi.";
                            bukberLocationStatus.style.color = "red";
                            bukberGetLocationBtn.disabled = false;
                            alert("Gagal mengambil lokasi: " + error.message);
                        }
                    );
        ...
        sendBukberOrderBtn.addEventListener('click', () => {
            ...
            let addressStr = '';
            if (serviceType === 'COD') {
                const address = bukberAddressInput.value.trim();
                const mapsLink = bukberMapsLinkInput.value;
                if (!address) {
                    alert('Mohon isi Alamat Lengkap untuk layanan COD!');
                    bukberAddressInput.focus();
                    return;
                }
                addressStr = `\nAlamat: ${address}`;
                if (mapsLink) {
